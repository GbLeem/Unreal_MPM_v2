...
void AMPM3D_NeoHookean_v2::SpawnBox()
{
	...
	for (int i = 0; i < NumParticles; ++i)
	{
		...
		Chaos::PMatrix<float, 3, 3> temp(1, 1, 1);
		Fs.Add(temp);
	}
...
void AMPM3D_NeoHookean_v2::P2G()
{
	for (int i = 0; i < NumParticles; ++i)
	{
		...
		Chaos::PMatrix<float, 3, 3> stress(0, 0, 0, 0, 0, 0, 0, 0, 0);

		Chaos::PMatrix<float, 3, 3> F = Fs[i];

		float J = F.Determinant();

		float volume = p->volume_0 * J;

		Chaos::PMatrix<float, 3, 3> F_T = F.GetTransposed();
		Chaos::PMatrix<float, 3, 3> F_inv_T = F_T.Inverse();
		//Chaos::PMatrix<float, 3, 3> F_inv_T = F_T.InverseFast();
		Chaos::PMatrix<float, 3, 3> F_minus_F_inv_T = F - F_inv_T;

		Chaos::PMatrix<float, 3, 3> P_term_0 = elastic_mu * F_minus_F_inv_T;
		Chaos::PMatrix<float, 3, 3> P_term_1 = elastic_lambda * log(J) * F_inv_T;
		Chaos::PMatrix<float, 3, 3> P = P_term_0 + P_term_1;

		stress = (1.f / J) * (P * F_T);

		Chaos::PMatrix<float, 3, 3> eq_16_term_0 = -volume * 2 * stress * dt; // [digit 2 is hyper parameter]
...
void AMPM3D_NeoHookean_v2::NewP2G()
{
	if (P2GFirst)
	{
		for (int i = 0; i < NumParticles; ++i)
...
void AMPM3D_NeoHookean_v2::G2P()
{
	for (int i = 0; i < NumParticles; ++i)
	{
		...
		Chaos::PMatrix<float, 3, 3> Fp_new(1, 0, 0, 0, 1, 0, 0, 0, 1);
